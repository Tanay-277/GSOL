...
// Default fallback assessment for when the API fails
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!genAI) {
          ...
          return NextResponse.json(
            { assessment: FALLBACK_ASSESSMENT, source: "fallback", error: "API key missing" },
            { status: 200 }
          );
        }
        ...
        const isValidResponses = responses.every((r: UserResponse) => 
            r && typeof r.question === 'string' && typeof r.answer === 'string'
            && r.question.trim() !== '' && r.answer.trim() !== ''
        );
        ...
        const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => reject(new Error('Model generation timeout')), 20000); // 20 seconds timeout
        });
        ...
        try {
            ...
            if (!assessment || assessment.trim().length < 100) {
                ...
                throw new Error("Invalid assessment generated");
            }
            ...
            return NextResponse.json(
                { assessment: FALLBACK_ASSESSMENT, source: "fallback", error: "Generation error" },
                { status: 200 }
            );
        }
    } catch (error) {
        ...
        // Return fallback assessment in case of any other error
        return NextResponse.json(
            { assessment: FALLBACK_ASSESSMENT, source: "fallback", error: errorMessage },
            { status: 200 }
        );
    }
}
